...
kf_PyList_Append::impl_call_pre (const call_details &cd) const
{
  ...
  const unaryop_svalue *unwrapped_sval
      = stored_sval->dyn_cast_unaryop_svalue ();
...
kf_PyList_Append::impl_call_post (const call_details &cd) const
{
  ...
  public:
                  ...
                  region_model_context *ctxt) const final override
    {
      ...
      // when it fails, if there's anything there, you should get rid of it
      // first.
  ...
  public:
                  ...
                  region_model_context *ctxt) const final override
    {
      ...
      const region *curr_reg
          = ob_item_ptr_sval->dyn_cast_region_svalue ()->get_pointee ();
      ...
      const svalue *refcnt_one_sval
          = mgr->get_or_create_long_cst (long_integer_type_node, 1);
      const svalue *new_refcnt_sval = mgr->get_or_create_binop (
          integer_type_node, PLUS_EXPR, curr_refcnt, refcnt_one_sval);
...
kf_PyList_New::impl_call_post (const call_details &cd) const
{
  ...
  {
  public:
    success_ (const call_details &cd) : call_info (cd) {}
                  ...
                  region_model_context *ctxt) const final override
    {
      ...
      // maybe_get_constant might return NULL_TREE which will then not give us
      // right result
      ...
      else // calloc
        {
          ...
          const region *ob_item_sized_region3
              = model->get_or_create_region_for_heap_alloc (prod_sval,
                                                            cd.get_ctxt ());
          // const region *ob_item_sized_region2 =
          // mgr->get_sized_region(ob_item_region, pyobj_ptr_tree, prod_sval);
          // const region *ob_item_sized_region =
          // mgr->get_sized_region(pylist_region, pyobj_ptr_tree, prod_sval);
      ...
      // tree allocated_tree = get_field_by_name(pylistobj_record,
      // "allocated"); const region *allocated_region =
      // mgr->get_field_region(pylist_region, allocated_tree);
      ...
      const svalue *refcnt_one_sval = mgr->get_or_create_long_cst (
          long_integer_type_node, 1); // TODO: switch to Py_ssize_t
...
kf_PyLong_FromLong::impl_call_post (const call_details &cd) const
{
  ...
  public:
                  ...
                  region_model_context *ctxt) const final override
    {
      ...
      const svalue *pyobj_svalue
          = mgr->get_or_create_unknown_svalue (pyobj_record);
      // const svalue *pyobj_unmergeable =
      // mgr->get_or_create_unmergeable(pyobj_svalue);
      const svalue *pylong_svalue
          = mgr->get_or_create_unknown_svalue (pylongobj_record);
      // const svalue *pylong_unmergeable =
      // mgr->get_or_create_unmergeable(pylong_svalue);

      // // Create a new region for PyLongObject.
      // tree tp_basicsize_field = get_field_by_name(pylongtype_vardecl,
      // "tp_basicsize"); tree comp_ref = build3(COMPONENT_REF,
      // TREE_TYPE(tp_basicsize_field),
      ...
      const svalue *tp_basicsize_sval
          = mgr->get_or_create_unknown_svalue (NULL);
      // const svalue *tp_basic_unmergeable =
      // mgr->get_or_create_unmergeable(tp_basicsize_sval);
      ...
      const svalue *pylong_type_ptr_sval = mgr->get_ptr_svalue (
          TREE_TYPE (pylongtype_vardecl), pylong_type_region);
      tree ob_type_field = get_field_by_name (pyobj_record, "ob_type");
      const region *ob_type_region
          = mgr->get_field_region (ob_base_region, ob_type_field);
...
kf_Py_Dealloc::impl_call_post (const call_details &cd) const
{
  ...
  //        pp_formatted_text(&pp)); const svalue *one_sval =
  //        mgr->get_or_create_int_cst(integer_type_node, 1);
  //        one_sval->dump_to_pp(&pp1, true);
  //        const svalue *new_ob_refcnt_sval =
  //        mgr->get_or_create_binop(integer_type_node, MINUS_EXPR,
  //        ob_refcnt_sval, one_sval); new_ob_refcnt_sval->dump_to_pp(&pp2,
  //        true);
...
cpython_analyzer_init_cb (void *gcc_data, void * /*user_data */)
{
  ...
  // PyDECREF is a macro that goes to _Py_DECREF in python 3.9 but Py_DECREF in
  // latest do _Py_Dealloc // check if this works in python 3.11 as well
